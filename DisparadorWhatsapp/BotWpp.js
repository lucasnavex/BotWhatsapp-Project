...
const {
  ...
  io.on("connection", function (socket) {
    socket.emit("message", "Disparador Iniciado!");
  ...
  /*app.post("/send-audio", async (req, res) => {
    ...
    if (cinco >= 12) {
      if (numberDDD <= 30) {
        ...
        client
          .sendMessage(numberZDG, media, { sendAudioAsVoice: true })
          .then((response) => {
            res.status(200).json({
              status: true,
              response: response,
  ...
  client.on("message", async (msg) => {
    if (
      ...
      client.sendMessage(
        msg.from,
        "Fico feliz que tenha se interessado em saber mais sobre!\n\nEstou encaminhando seu numero para nossa equipe especializada e ja entrarão em contato com você."
      );
      client.sendMessage(
        "553288429582@c.us",
